...
> **Loom-team note (Web-only migration):** This is the upstream dual-backend design
> record (source commit `fdc89ece`). Loom-team ships only the TypeScript/Web half:
> all Rust references below (`crates/routa-core`, `crates/routa-server`, Axum
> handlers, Rust RPC/REST validation) describe the removed Rust backend and are
> kept for provenance only. The implemented Web surface is
> `src/core/kanban/task-attachments.ts`, `src/app/api/tasks/**`, the kanban/team
> input components, and `src/core/acp/prompt-content.ts`.
